<?php
/**
 * Email notifications for Content Alerts & Subscriptions.
 */
...
class Content_Alerts_Subscriptions_Emails
{
    ...
    public function maybe_send_term_publish_emails($new_status, $old_status, $post)
    {
        ...
        foreach ($terms_to_notify as $term_id) {
            $subscribers = Content_Alerts_Subscriptions_DB::get_subscribers($term_id, 'term');
        ...
        $email_options = get_option('content_alerts_subscriptions_email_settings');
        $subject_template = isset($email_options['taxonomy_notification_email_subject']) ? $email_options['taxonomy_notification_email_subject'] : __('New Post in {taxonomy_name}: {post_title}', 'content-alerts-subscriptions');
        $content_template = isset($email_options['taxonomy_notification_email_content']) ? $email_options['taxonomy_notification_email_content'] : __("Hi {user_name},\n\nA new post has been published in {taxonomy_name}: {post_title}.\n\nCheck it out here: {post_link}", 'content-alerts-subscriptions');
    ...
    {
        $subscribers = Content_Alerts_Subscriptions_DB::get_subscribers($object_id, $object_type);
        if (empty($subscribers)) {
            return; // No one to email
        }

        $options = get_option('content_alerts_subscriptions_email_settings');
        $subject_template = isset($options['notification_email_subject']) ? $options['notification_email_subject'] : __('Update: {post_title}', 'content-alerts-subscriptions');
    ...
    public function send_welcome_email($user_id, $object_id, $object_type)
    {
        ...
        $options = get_option('content_alerts_subscriptions_email_settings');
        $subject_template = isset($options['welcome_email_subject']) ? $options['welcome_email_subject'] : __('Welcome to {post_title} Updates!', 'content-alerts-subscriptions');
        $content_template = isset($options['welcome_email_content']) ? $options['welcome_email_content'] : __("Hi {user_name},\n\nYou are now subscribed to updates for {post_title}.\n\nView it here: {post_link}\n\nThanks!", 'content-alerts-subscriptions');
    ...
    public function send_unsubscribe_email($user_id, $object_id, $object_type)
    {
        ...
        $options = get_option('content_alerts_subscriptions_email_settings');
        $subject_template = isset($options['unsubscribe_email_subject']) ? $options['unsubscribe_email_subject'] : __('Unsubscribed from {post_title}', 'content-alerts-subscriptions');
        $content_template = isset($options['unsubscribe_email_content']) ? $options['unsubscribe_email_content'] : __("Hi {user_name},\n\nYou have successfully unsubscribed from updates for {post_title}.", 'content-alerts-subscriptions');
